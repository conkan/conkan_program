...
-- Created on Fri May 29 16:38:58 2015
-- 
...
CREATE TABLE pg_cast (
  ...
  CONSTRAINT pg_cast_fk_pgid FOREIGN KEY (pgid) REFERENCES pg_reg_program (pgid) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB;
...
CREATE TABLE pg_equip (
  ...
  CONSTRAINT pg_equip_fk_pgid FOREIGN KEY (pgid) REFERENCES pg_reg_program (pgid) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB;
...
CREATE TABLE pg_program (
  ...
  updateflg varchar(64) NULL,
  INDEX pg_program_idx_pgid (pgid),
  INDEX pg_program_idx_roomid (roomid),
  INDEX pg_program_idx_staffid (staffid),
  PRIMARY KEY (id),
  CONSTRAINT pg_program_fk_pgid FOREIGN KEY (pgid) REFERENCES pg_reg_program (pgid) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
CREATE TABLE pg_progress (
  ...
  INDEX pg_progress_idx_pgid (pgid),
  INDEX pg_progress_idx_staffid (staffid),
  PRIMARY KEY (id),
  CONSTRAINT pg_progress_fk_pgid FOREIGN KEY (pgid) REFERENCES pg_reg_program (pgid) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
CREATE TABLE pg_reg_cast (
  ...
  name varchar(64) NOT NULL,
  namef varchar(64) NULL,
  entrantregno integer unsigned NULL,
  needreq enum('req', 'accepted', 'innego', 'yetnego') NULL DEFAULT 'accepted',
  needguest enum('Y', 'N') NULL DEFAULT 'N',
  INDEX pg_reg_cast_idx_pgid (pgid),
  PRIMARY KEY (id),
  CONSTRAINT pg_reg_cast_fk_pgid FOREIGN KEY (pgid) REFERENCES pg_reg_program (pgid) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB;
...
CREATE TABLE pg_reg_equip (
  ...
  INDEX pg_reg_equip_idx_pgid (pgid),
  PRIMARY KEY (id),
  CONSTRAINT pg_reg_equip_fk_pgid FOREIGN KEY (pgid) REFERENCES pg_reg_program (pgid) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB;
...
CREATE TABLE pg_reg_program (
  ...
  comment text NULL,
  updateflg varchar(64) NULL,
  PRIMARY KEY (pgid, layout)
) ENGINE=InnoDB;

DROP TABLE IF EXISTS pg_regist_conf;

--
-- Table: pg_regist_conf
--
CREATE TABLE pg_regist_conf (
  jsonkeyid varchar(64) NOT NULL,
  hashkey varchar(64) NOT NULL,
  db_name varchar(128) NOT NULL,
  valtype varchar(64) NOT NULL,
  upperkeyid varchar(64) NULL,
  upperkeyval varchar(64) NULL,
  INDEX pg_regist_conf_idx_upperkeyid (upperkeyid),
  PRIMARY KEY (jsonkeyid),
  UNIQUE HashKey_UNIQUE (hashkey),
  CONSTRAINT pg_regist_conf_fk_upperkeyid FOREIGN KEY (upperkeyid) REFERENCES pg_regist_conf (jsonkeyid) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB;
