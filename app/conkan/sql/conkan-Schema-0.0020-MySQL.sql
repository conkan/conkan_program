...
-- Created on Fri Mar  3 21:51:13 2017
-- 
...
CREATE TABLE pg_all_equip (
  ...
  suppliers varchar(64) NULL,
  updateflg varchar(64) NULL,
  rmdate datetime NULL,
  INDEX pg_all_equip_idx_roomid (roomid),
  PRIMARY KEY (equipid),
  UNIQUE equipNo_UNIQUE (equipno),
  CONSTRAINT pg_all_equip_fk_roomid FOREIGN KEY (roomid) REFERENCES pg_room (roomid) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB;
...
CREATE TABLE pg_reg_program (
  ...
  comment text NULL,
  originaljson text NULL,
  cloudtag varchar(128) NULL,
  updateflg varchar(64) NULL,
  PRIMARY KEY (regpgid)
) ENGINE=InnoDB;
